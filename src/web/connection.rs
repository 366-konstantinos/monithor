...
use std::{io::{self, Cursor}, error::Error};
use tokio::io::{AsyncReadExt, AsyncWriteExt, BufWriter};
use actix_web::rt::net::{TcpStream, TcpListener};
use anyhow;
use crate::frame::{Frame, Error::Incomplete};
...
impl Connection {
    ...
    async fn write_value(&mut self, frame: String) -> io::Result<()> {
        self.stream.write_u8(b'+').await?;
        // match frame {
        //     Frame::Simple(val) => {
        //         self.stream.write_u8(b'+').await?;
