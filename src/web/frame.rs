...
impl Frame {
    ...
    pub fn check(src: &mut Cursor<&[u8]>) -> Result<(), Error> {
        ...
        match get_u8(src)? {
            ...
            actual => Ok(()) // Err(format!("protocol error; invalid frame type byte `{}`", actual).into()),
        }
    }
    ...
    pub fn parse(src: &mut Cursor<&[u8]>) -> anyhow::Result<Frame, Error> {
        match get_u8(src)? {
            ...
            _ => {
                // Read the line and convert it to `Vec<u8>`
                let line = get_line_new(src).unwrap().to_vec();
    ...
    pub(crate) fn to_error(&self) -> anyhow::Error {
        anyhow::format_err!("unexpected frame: {}", self).into()
    }
}
...
fn get_line_new<'a>(src: &mut Cursor<&'a [u8]>) -> Result<&'a [u8], Error> {
    // Scan the bytes directly
    let start = src.position() as usize;
    // Scan to the second to last byte
    let end = src.get_ref().len() - 1;
...
impl From<String> for Error {
    fn from(src: String) -> Error {
        Error::Other(anyhow::format_err!(src).into())
    }
}
